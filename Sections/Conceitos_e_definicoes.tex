...
%Em dois artigos apresenta "modelling tool"

%Redes de petri são utilizadas como ferramentas de modelagem gráfica com propriedades matemática. 
...
\par
A grade premissa desta  é descrever sistemas de processamento de informações caracterizados como concorrentes, assíncronos, distribuídos, paralelos, não deterministas e/ou estocásticos~\cite{murata1989petri}.
...
% 
A relação criada pela frase poderia a ser analisada pela lógica proposicional\cite{abe2002introducao}\todo{Descrever como seria está analise}.
...
Linguagem de especificação de propriedades, do inglês \textit{Property Specification Language} - PSL, é uma notação formal para especificação de comportamento em sistemas eletrônicos, compatível com as linguagens VHDL, Verilog, SystemC e SystemVerilog\todo{Adicionar referência a cada Ling citada}. Destina-se a utilização para a especificação funcional, mas também para entrada de ferramentas de verificação. A especificação do PSL é a utilização de assertivas sobre propriedades de um sistema, sendo estas propriedades constituídas de três elementos: expressões boolenas, expressões sequenciais e operadores temporais~\cite{IEEEPSL}.

\todo[inline]{Corrigir a apresentação da citação IEEE... estranho}
...
% Entre os pontos principais do PSL encontram-se:
% \begin{itemize}
...
\todo[inline]{Apresentar exemplos de uso da PSL}
...
\todo[inline]{Descrever qual o objetivo da assert apresentado}
...
\todo[inline]{Adicionar exemplos de propriedades de segurança em hardware}
...
\todo[inline]{Apresentar um exemplo de como e utilizado em código}
...
Na \autoref{fig:elimicacao_codigo}, supondo que os nós $a,b$ e $c$ estejam ativa, porém o nó é\todo{Corrigir nó é?} não esteja, o último pode ser eliminado imediatamente. Os nós restantes continuam no GDA, a menos que não tenho variáveis ativas atreladas aos nós~\cite{aho2007compilers}.
...
\todo[inline]{Apresentar exemplos}
