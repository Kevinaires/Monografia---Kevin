...
O método proposto 
% consiste na análise de código VHDL de um circuito lógico através de transformação de código e da ferramenta ESBMC. O método 
...
% \par
% A adição das assertivas pode ser realizada de maneira automatizada ou manual inserida pelo desenvolvedor de hardware, caso necessite realizar uma verificação mais especifica sobre o código. 
...
\par
% Na \autoref{fig:assertiva} a mensagem a ser exibida em caso de erro é "O resultado foi diferente de 0" e a severidade definida como \textit{error} indicando que é uma assertiva fatal. 
% Na atual implementação da ferramenta não foram utilizadas, sendo a próxima etapa do processo, onde será utilizado um framework para teste de unidade.

\par
% Outra propriedade utilizada 
...
\par
% Com a assertiva passada para a função de busca, 
...
\par
Segundo, \citeonline{rocha2015verificaccao}, a função \texttt{\_\_VERIFIER\_nondet\_int()} tem a função de modelar valores inteiros não deterministicos e é importante no desenvolvimento do método, pois evita erros, onde dado estado do código não pode ser alcançado, devido a variável não inicializada.
...
\par
O \textit{model checker} adotado no desenvolvimento de método é o ESBMC\todo{referência} na versão $3.0.0$ e conforme explicado na \autoref{cap:bounded}, esta ferramenta recebe como entrada um código C ou C$++$ e devido a isso foi escolhida neste projeto\todo{apresentar outras vantagens}. Nesta seção será apresentado a \textbf{etapa D} do métod proposto na \autoref{fig:Fluxo_ferramenta}.
...
% e através desta a busca das claims, 
...
% Com estas opções o número de claims geradas é reduzida e desta forma gerando um benificio ao programa, que realiza uma quantidade menor de checagem das claims. 
% Com resultado gerado pelo ESBMC é realizado um busca pelas claims com assertivas e gerado uma lista com apenas com estas claims, para posterior análise individual delas.
\begin{figure}[H]
	\begin{center}
...
\todo[inline]{Ajustar os códigos, alguns estao muito grandes e com trechos desnecessário, não foi citado o link do repo da ferramenta}